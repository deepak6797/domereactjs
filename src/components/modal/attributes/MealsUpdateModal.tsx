...
import { useUpdateMeals } from "../../../hooks/meals.hook";
...
interface MealsUpdateModalProps {
  visible: boolean;
  isPending?: boolean;
  data?: any;
  onClose: () => void;
...
const MealsUpdateModal: React.FC<MealsUpdateModalProps> = ({
  visible,
  isPending,
  onClose,
  setVisible,
  ...
  const { mutateAsync: updateMeals } = useUpdateMeals();

  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    const resData: Props = data as Props;
    try {
      await updateMeals({ body: resData, id: id });
      showSuccessMessage("Meals updated successfully !!!");
      setVisible(false);
    } catch (err) {
