...
import { useUpdateAgeRange } from "../../../hooks/agerange.hook";
...
interface AgeRangeUpdateModalProps {
  visible: boolean;
  isPending?: boolean;
  data?: any;
  onClose: () => void;
...
const AgeRangeUpdateModal: React.FC<AgeRangeUpdateModalProps> = ({
  visible,
  isPending,
  onClose,
  setVisible,
  ...
  const { mutateAsync: updateAgeRangeName } = useUpdateAgeRange();

  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    const resData: Props = data as Props;
    try {
      await updateAgeRangeName({ body: resData, id: id });
      showSuccessMessage("ageRange updated successfully !!!");
      setVisible(false);
    } catch (err) {
...
export default AgeRangeUpdateModal;
