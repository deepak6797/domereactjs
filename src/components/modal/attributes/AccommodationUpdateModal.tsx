...
import { useUpdateAccommodation } from "../../../hooks/accommodation.hook";
...
interface AccommodationUpdateModalProps {
  visible: boolean;
  isPending?: boolean;
  data?: any;
  onClose: () => void;
...
const AccommodationUpdateModal: React.FC<AccommodationUpdateModalProps> = ({
  visible,
  isPending,
  onClose,
  setVisible,
  ...
  const { mutateAsync: updateAccommodation } = useUpdateAccommodation();

  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    const resData: Props = data as Props;
    try {
      await updateAccommodation({ body: resData, id: id });
                  ...
                  <div className="grid grid-cols-6 gap-6">
                    <div className="col-span-6">
                      <CustomInput
                        ...
                        type="text"
                        name="name"
                        placeHolder="Update accommodation"
                        required={true}
                      />
                    </div>
                  </div>
