...
import { useAddCountry } from "../../hooks/country.hook";
import CustomSelectContinent from "../form/custom/CustomSelect";
...
const CountryModal: React.FC<CountryModalProps> = ({
  ...
  const { data: continentData } = useGetAllContinent();
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data: any) => {
    ...
    try {
      if (continentId) {
        const response = await addCountry({ body: resData, id: continentId });
                ...
                <div className="p-6 space-y-6">
                  <div className="grid grid-cols-6 gap-6">
                    <div className="col-span-6">
                      <CustomSelectContinent
                        name="continent"
                        options={getValue(continentData, "results", [])}
