...
const WhatsExcluded: React.FC<Props> = ({
  placeHolder,
  type,
  required,
  excludedList,
  ...
  const [highLightsLength, setHighLightsLength] = useState(excludedList.length)
  ...
  const handleAddHighlight = () => {
    if (newHighlight.trim() !== "") {
      ...
      setHighlights((prev:any)=>({...prev, [highLightsLength]: newHighlight}))
      setNewHighlight("");
  ...
  const handleEditHighlight = (index: number) => {
    setEditIndex(index);
    setNewHighlight(excludedList[index]);
